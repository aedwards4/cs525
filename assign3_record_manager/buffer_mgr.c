...
{	
	...
	int lhIdx = 0; 
	int lhNum; 
	while(idx < bufferCapacity){ // loop over pages
		if(pages[idx].clientCount == 0){ // check if page is already used by clients
	...
	idx = lhIdx + 1;
	while( idx < bufferCapacity){ // loop over pages of buffer pool
	...
	if(pages[lhIdx].dirBits == 1){ // check for dirty pages
 ...
 * @return RC 
 */
extern RC markDirty (BM_BufferPool *const bufferPool, BM_PageHandle *const pgHandler)
{
	pgs *pages = (pgs *)bufferPool->bookInfo; // initialize page frame with buffer pool book info
	int idx = 0; // assign initial index to zero
    if(bufferCapacity == 0){ // null and empty check for buffer size
       return RC_ERROR; // if buffer size is zero or null return error 
	}		
    while(idx < bufferCapacity){ // iterate over pages in the buffer
	 	if(pages[idx].pageNum == pgHandler->pageNum){ // // current page 
...
extern void queue(BM_BufferPool *const bufferPool, pgs *newPage)
{
	...
	while(idx < bufferCapacity){  // loop over pages in the buffer 
		if(pages[fPointer].clientCount != 0){ // check if page is access by any client
			fPointer++; // move to next frame 
			if(fPointer % bufferCapacity == 0){ // check if front pointer reaches max capacity
               fPointer = 0; // reset pointer
			}		
		}
		else{	
			if(pages[fPointer].dirBits == 1){ // check if frame already used
				SM_FileHandle fh;
 ...
 * @return RC 
 */
extern RC initBufferPool(BM_BufferPool *const bufferPool, const char *const pagesName, const int count, ReplacementStrategy stg, void *intInfo)
{	
...
extern RC shutdownBufferPool(BM_BufferPool *const bufferPool)
{
	...
	int idx= 0;	// initialize index to zero
	while(idx < bufferCapacity){ // start with zero index untill reaches to buffer size
		if(pages[idx].clientCount != 0){ // page used by client
 ...
 * @return RC 
 */
extern RC unPinPage(BM_BufferPool *const bufferPool, BM_PageHandle *const pgHandler)
{	
	pgs *pages = (pgs *)bufferPool->bookInfo; // initialize page frame
	int idx = 0; // initialize index value by one
	while(idx < bufferCapacity){ // loop over pages in the buffer pool
...
extern RC forceFlushPool(BM_BufferPool *const bufferPool)
{
	...
	while(idx < bufferCapacity){ // loop over pages untill index equals capacity
		if(pages[idx].clientCount == 0 && pages[idx].dirBits == 1){ // check client count and dirty bit flag
			SM_FileHandle fh;
...
extern PageNumber *getFrameContents (BM_BufferPool *const bufferPool)
{
	...
	while(idx < bufferCapacity) { // loop over pages
		if(pages[idx].pageNum != -1){ // page should not be empty
 ...
 * @return RC 
 */
extern RC pinPage (BM_BufferPool *const bufferPool, BM_PageHandle *const pgHandler, const PageNumber pageNum)
{
	SM_FileHandle fh;
	pgs *pages = (pgs *)bufferPool->bookInfo;
	bool isBufferFull = true;
	if(pages[0].pageNum != -1) { // buffer pool should not be empty
		int idx = 0; // initialize index with zero 
		while(idx < bufferCapacity){ // loop over bufferpool to pin pages
			if(pages[idx].pageNum == -1){ // incase buffer pool is not empty 
				openPageFile(bufferPool->file, &fh); // open file
			...
			if(pages[idx].pageNum == pageNum){ // current page number and Pages page number equal
		...
		if(isBufferFull){ // incase buffer pool is full  
			...
			if(bufferPool->replaceStgy == 0){ // if starttegy is queue
                queue(bufferPool, new_page); // call queue functionality
				new_page->usedTimes = 1; // assign dirty flag
			}else if(bufferPool->replaceStgy == 1){ // starategy is lru
